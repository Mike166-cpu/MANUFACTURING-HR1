...
exports.uploadDocument = async (req, res) => {
  try {
    const { employee_id, document_url, request_id } = req.body;

    if (!employee_id || !document_url || !request_id) {
    ...
    global.io.emit(`notification-admin`, {
      message: `Employee ${employee_id} submitted a document for approval.`,
      employee_id,
      request_id,
      dashboard: "admin",
    });
...
exports.getUploadedDocumentsByEmployeeId = async (req, res) => {
  try {
    ...
    const documents = await UploadedDocument.find({ employee_id });
...
exports.updateUploadedDocument = async (req, res) => {
  try {
    ...
    global.io.emit(`notification-admin`, {
      ...
      employee_id: updatedDocument.employee_id,
