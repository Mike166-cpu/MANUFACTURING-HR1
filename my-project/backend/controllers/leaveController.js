...
const hasActiveLeave = async (employee_id) => {
...
exports.fileLeave = async (req, res) => {
  try {
    ...
    const activeLeave = await hasActiveLeave(employee_id);
    if (activeLeave) {
    ...
    global.io.emit('notification-admin', {
      message: `Employee ${employee_firstname} requested a leave.`,
      employee_id,
      leave_id,
      dashboard: "admin"
    });
...
exports.updateLeaveStatus = async (req, res) => {
  try {
    ...
    if (status === "Approved") {
      const leaveBalance = await LeaveBalance.findOne({ employee_id: leave.employee_id });
      if (!leaveBalance) {
        return res.status(404).json({ message: "Leave balance not found" });
      }
