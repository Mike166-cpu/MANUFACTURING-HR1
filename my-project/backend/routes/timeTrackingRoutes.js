...
const timeTrackingController = require('../controllers/timeTrackingController'); // Import the controller
...
router.post("/time-in", async (req, res) => {
    try {
        ...
        if (!schedule.days.includes(currentDay)) {
            return res.status(400).json({ message: "You are not scheduled to work today" });
        }

        // Check if the current time is within the allowed time-in window (8 AM to 5 PM)
        if (currentHour < 8 || currentHour >= 17) {
            return res.status(400).json({ message: "You can only time in between 8 AM and 5 PM" });
        }
...
router.put("/time-out", async (req, res) => {
    try {
        ...
        let totalHours = (activeSession.time_out - activeSession.time_in) / (1000 * 60 * 60);
        
        // Check if time out is after 5 PM
        const timeOutHour = activeSession.time_out.getHours();
        if (timeOutHour >= 17) {
            const overtime = timeOutHour - 17 + (activeSession.time_out.getMinutes() / 60);
